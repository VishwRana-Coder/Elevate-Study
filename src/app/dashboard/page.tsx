...
import type { OnboardingData } from "@/app/user-info/page";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="card rounded-2xl p-6 w-full max-w-sm text-center shadow-2xl relative">
        ...
        <h2 className="font-black text-lg mb-4"><LuTimer />
Focus Session</h2>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="card rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-black text-base flex gap-1"> Manage Subjects</h2>
        ...
        <div className="space-y-2 mb-4 max-h-64 overflow-y-auto">
          {list.map((s,i) => (
            <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[var(--surface-secondary)] border border-[var(--border)]">
              ...
              <button onClick={() => { setEditIdx(i); setEditVal(s); }} className="text-[var(--foreground)]/40 hover:text-[var(--primary)] text-xs px-1"><MdModeEditOutline /></button>
              <button onClick={() => setList(list.filter((_,j)=>j!==i))} className="text-[var(--foreground)]/40 hover:text-red-500 text-xs px-1"><FaTrashAlt /></button>
            </div>
          ))}
        </div>
      ...
      <div className="card rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-black text-base flex items-center gap-2">
            {event?.id ? (
              <>
                <MdModeEditOutline className="text-[var(--primary)] text-sm" />
                Edit Event
              </>
            ) : (
              "➕ New Event"
          ...
          <button
            onClick={onClose}
            className="text-[var(--foreground)]/40 hover:text-[var(--foreground)] text-lg"
          >
            ✕
          </button>
        </div>
...
// ─── Main ─────────────────────────────────────────────────────────────────────
export default function Dashboard({ user, userName }: any) {
  ...
  const nextId = useRef(200);
    ...
    const profile = lsGet<OnboardingData | null>("es_onboarding", null);

  const resolvedUser = clerkUser || user;
  const resolvedName =
    resolvedUser?.username ||
    resolvedUser?.firstName ||
    profile?.displayName;

  const firstLetter = resolvedName?.[0];
  const userGraduation = profile?.graduationYear;
  const userSchool = profile?.schoolLevel;
  const userStudyGoal = profile?.studyGoal;
  const currentHour = new Date().getHours();



  useEffect(() => {
    if (!clerkUser && !resolvedName) {
      router.replace("/");
    }
  }, [clerkUser, resolvedName, router]);
  ...
  // ── Nav items ─────────────────────────────────────────────────────────────
  const NAV = [
    {key:"dashboard" as Page, icon:<MdOutlineDashboard />, label:"Dashboard"},
    {key:"tasks"     as Page, icon:<FaTasks />, label:"Tasks"},
    {key:"goals"     as Page, icon:<PiTargetBold />, label:"Goals"},
    {key:"calendar"  as Page, icon:<FaCalendarAlt />, label:"Calendar"},
    {key:"analytics" as Page, icon:<SiGoogleanalytics />, label:"Analytics"},
  ];
  ...
  if(hydrated){
    return (
      <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] flex transition-colors duration-300 overflow-hidden">
        ...
        {/* ══ SIDEBAR ══ */}
        <aside 
          className={`fixed top-0 left-0 z-40 h-screen flex flex-col flex-shrink-0 bg-[var(--sidebar)] text-white transition-all duration-300 overflow-y-auto
          ...
          <div className="p-3 border-t border-white/10 flex-shrink-0">
            <div className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-white/10 transition-colors cursor-pointer">
              {clerkUser ? (
                ...
                <div
                  className="flex items-center justify-center font-semibold text-white rounded-full w-10 h-10 bg-gradient-to-br from-[var(--primary)] to-[var(--accent)]"
                >
                  {firstLetter}
                </div>
              )}      
              {sidebarOpen && <div className="min-w-0" ><p className="text-xm font-semibold truncate">{resolvedName}</p></div>}
            </div>
          </div>
        </aside>
        ...
        {/* ══ MAIN ══ */}
        <div 
          className={`flex-1 flex flex-col min-w-0 overflow-hidden min-h-screen transition-all duration-300
            ${sidebarOpen ? "ml-0 md:ml-56" : "ml-0 md:ml-16"}`}
        >
          ...
          <header className="flex items-center justify-between px-4 md:px-6 py-3 border-b border-[var(--border)] bg-[var(--background)] flex-shrink-0 gap-3">
            ...
            <h1 className="font-black text-sm md:text-lg leading-tight truncate">
              {page === "dashboard"
                ? `Good ${greeting}, ${resolvedName}`
                : NAV.find((n) => n.key === page)?.label}
            </h1>

            {page === "dashboard" && (
              <div className="text-[10px] md:text-xs text-[var(--foreground)]/50 hidden sm:block">
              <div>{todayDate} · {streak} day streak </div>
              </div>
            )}
          </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <button onClick={()=>setShowPomodoro(true)}
                ...
                <LuTimer /><span className="hidden md:inline">Focus</span>
              </button>
              <button onClick={()=>setShowPomodoro(true)} className="sm:hidden w-8 h-8 rounded-lg bg-[var(--surface)] border border-[var(--border)] flex items-center justify-center text-sm"><LuTimer />
  </button>
              {/* Theme toggle */}
              <button onClick={toggleTheme} aria-label="Toggle theme"
                ...
                <span className={`absolute top-0.5 w-5 h-5 rounded-full flex items-center justify-center text-[10px] transition-all duration-300 shadow-sm ${dark?"translate-x-6 bg-[var(--primary)]":"translate-x-0.5 bg-[var(--primary)]  text-[#fff]"}`}>
                  {dark? <FaMoon /> : <IoSunny />
  }
                </span>
              </button>
            ...
            {page==="dashboard" && (
              <div className="p-4 md:p-6 space-y-4 max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    { icon: <FaClock />, label:"Today", val:`${Math.floor(todayMinutes/60)}h ${todayMinutes%60}m` },
                    { icon: <FaFire />, label: "Streak", val: `${streak} ${streak === 1 ? "day" : "days"}` },                  
                    { icon: <RiTaskFill />, label:"Done", val:`${tasks.filter(t=>t.done).length}` },
                    { icon: <FaCalendarAlt/>, label:"This week", val:`${Math.floor(weekMinutes/60)}h` },
                  ].map(s=>(
  ...
  Start Focus
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          {editingGoal ? (
                            ...
                            <button
                              ...
                              className="text-xs font-bold text-[var(--primary)] hover:underline px-1.5 py-1 rounded-lg flex gap-0.5 hover:bg-[var(--hover)] transition-colors"
                              title="Click to edit your daily goal"
                            >
                              {Math.floor(studyGoalMinutes/60)}h {studyGoalMinutes%60>0?`${studyGoalMinutes%60}m`:""} <div> <MdModeEditOutline size={16}/></div>
                            </button>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[{icon: <FaTasks />,label:"All Tasks",to:"tasks"},{icon:<PiTargetBold />,label:"Goals",to:"goals"},{icon:<FaCalendarAlt />,label:"Calendar",to:"calendar"},{icon: < SiGoogleanalytics/>,label:"Analytics",to:"analytics"}].map(q=>(
                        <button key={q.label} onClick={()=>navigate(q.to as Page)}
                          ...
                          <span className="text-xl block mb-1 group-hover:scale-110 transition-transform flex items-center justify-center">{q.icon}</span>
                          <span className="text-xs font-semibold hover:text-[13px] transition-all">{q.label}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="card rounded-2xl p-5">
                      <div className="flex items-center justify-between mb-3">
                        <h2 className="font-black text-sm flex gap-1 items-center "><FaCalendarAlt /> Upcoming</h2>
                    ...
                    <div className="card rounded-2xl p-5">
                      <div className="flex items-center justify-between mb-3">
                        <h2 className="font-black text-sm flex gap-1 items-center"><PiTargetBold /> Goals</h2>
                    ...
                    <div className="rounded-2xl p-5 bg-gradient-to-br from-[var(--primary)]/10 to-[var(--accent)]/10 border border-[var(--primary)]/20">
                      <div className="text-[10px] font-black uppercase tracking-wider mb-2 flex gap-1">
                        <div><BsChatQuoteFill size={13} color="#fff"/></div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <div className="flex items-center gap-2 flex-1 min-w-36">
                        ...
                        <button onClick={()=>setShowSubjectMgr(true)} title="Manage subjects"
                          className="w-9 h-9 rounded-xl bg-[var(--surface-secondary)] border border-[var(--border)] hover:bg-[var(--hover)] transition-colors flex items-center justify-center text-sm flex-shrink-0"> <MdModeEditOutline /></button>
                      </div>
                      ...
                      <div key={t.id} className="flex items-center gap-3 px-4 py-3 hover:bg-[var(--hover)] transition-colors group">
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                          <button onClick={()=>{setEditingTaskId(t.id);setEditDraft(t.label);}}
                            className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--foreground)]/40 hover:text-[var(--primary)] hover:bg-[var(--primary)]/10 transition-colors text-xs"> <MdModeEditOutline /></button>
                          <button onClick={()=>deleteTask(t.id)}
                            className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--foreground)]/40 hover:text-red-500 hover:bg-red-500/10 transition-colors text-xs"><FaTrashAlt /></button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-4">
                ...
                <div className="card rounded-2xl p-5">
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    <button onClick={addGoal} className="primary-btn px-5 py-2.5 rounded-xl text-sm font-bold hover:opacity-90 transition-opacity">Add Goal</button>
                  </div>
                </div>
                    ...
                    {Array.from({length:calDaysInMonth}).map((_,i)=>{
                      ...
                      return (
                        <div key={day} onClick={()=>setEventModal({event:{date:ds}})}
                          className={`relative rounded-lg flex flex-col items-center pt-1 pb-1 text-xs font-semibold cursor-pointer transition-colors min-h-[36px] md:min-h-[44px] ${isToday?"bg-[var(--accent)] text-white":"hover:bg-[var(--hover)] text-[var(--foreground)]"}`}>
                          <span>{day}</span>
                          {dayEvts.length>0&&(
                      ...
                      <div key={ev.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl border group ${EVT_STYLE[ev.type]}`}>
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                          ...
                          <button onClick={()=>deleteEvent(ev.id)}
                            className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-red-500/20 transition-colors text-xs"><FaTrashAlt /></button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {page==="analytics" && (
              <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {[
                    {icon: <FaClock />,label:"Today",     val:`${Math.floor(todayMinutes/60)}h ${todayMinutes%60}m`, sub:"study time"},
                    {icon: <FaFire />,label:"Streak",     val:`${streak} days`,                                sub:streak>=7?"🏆 Week streak!":"Keep going!"},
                    {icon: <RiTaskFill />,label:"Tasks Done", val:`${completedThisWeek}`,                          sub:`${tasks.filter(t=>!t.done&&t.group!=="upcoming").length} remaining`},
                    {icon: <FaCalendarAlt />,label:"This Week",  val:`${Math.floor(weekMinutes/60)}h`,               sub:`${weekMinutes}min total`},
                  ].map(s=>(
                ...
                <div className="card rounded-2xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="font-black text-base flex items-center gap-1"><FaFire /> Study Streak — Last 7 Days</h2>
                    ...
                    {Array.from({length:7}).map((_,i)=>{
                      ...
                      return (
                        <div key={i} className="flex-1 text-center">
                          <div className={`h-8 md:h-10 rounded-lg mb-1.5 transition-all ${active?"bg-[var(--accent)]":"bg-[var(--surface-secondary)] border border-[var(--border)]"}`}
                            title={`${ds}: ${mins}min`}/>
                        ...
                        return (
                          <div key={i} className="flex-1 flex flex-col items-center gap-1">
                            ...
                            <div className="w-full flex items-end" style={{height:"80px"}}>
                              <div className={`w-full rounded-t-lg transition-all duration-700 ${isToday?"bg-[var(--accent)]":mins>0?"bg-[var(--primary)]/40":"bg-[var(--border)]"}`}
